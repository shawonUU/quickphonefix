                ...
                <div class="col-lg-12">
                  <div class="card">
                    <div class="card-header align-items-center d-flex">
                      <h4 class="card-title mb-0 flex-grow-1">Edit Book</h4>
                    ...
                    <div class="card-body">
                      <div class="live-preview">
                        <div class="row gy-4">
                            <form action="{{ route('books.update', $product->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('PATCH')
                                <div class="row">
                                    ...
                                    <div class="col-12 col-md-4 col-lg-3 p-2 mb-3">
                                        ...
                                        <select name="subject[]" id="" class="form-select mb-3 select2" multiple required>
                                            <option value="">--Select--</option>
                                            @foreach($subjects as $subject)
                                                <option value="{{$subject->id}}" {{ in_array($subject->id , explode(',', $product->subject_id)) ? 'selected' : ''}}>{{$subject->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-4 col-lg-3 p-2 mb-3">
                                        <label for="" class="form-label ">Writer</label>
                                        <select name="writer[]" id="" class="form-select mb-3 select2" multiple required>
                                            <option value="">--Select--</option>
                                            @foreach($writers as $writer)
                                                <option value="{{$writer->id}}" {{ in_array($writer->id , explode(',', $product->writer_id)) ? 'selected' : ''}}>{{$writer->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-4 col-lg-3 p-2 mb-3">
                                        <label for="" class="form-label">Publisher</label>
                                        <select name="publisher[]" id="" class="form-select mb-3 select2" multiple required>
                                            <option value="">--Select--</option>
                                            @foreach($publishers as $publisher)
                                                <option value="{{$publisher->id}}" {{ in_array($publisher->id , explode(',', $product->publisher_id)) ? 'selected' : ''}}>{{$publisher->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-12 col-md-4 col-lg-3 p-2 mb-3">
