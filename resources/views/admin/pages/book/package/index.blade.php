                    ...
                    <div class="card-header align-items-center d-flex">
                      ...
                      <div class="flex-shrink-0">
                        <div class="form-check form-switch form-switch-right form-switch-md">
                            <a href="{{ route('books.create_package') }}" class="btn btn-info">Create Book</a>
                        </div>
                      </div>
                    </div>
                    <!-- end card header -->
                        ...
                        <div class="row gy-4">
                            <table class="table" id="dataTbl">
                                ...
                                <tr>
                                    ...
                                    <td>{{ $item->name }}</td>
                                    <td>{{$item->book_names}}</td>
                                    <td>{{ $item->price }}</td>                                 
                                    <td>{{ $item->offer_price }}</td>                                 
                                    <td>{{ $item->offer_from }}</td>                                 
                                    <td>{{ $item->offer_to }}</td>                                 
                                    <td class="{{ $item->status=='1'?'text-danger':'' }}">{{ $item->status=='1'?'Active':'Inactive' }}</td>
                                      <td>
                                        <a href="{{ route('books.package_edit',$item->id) }}"><button type="button" class="btn btn-sm btn-primary waves-effect waves-light"><i class="ri-ball-pen-line" style="color: #fff"></i> </button></a>
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <form
                                                        action="{{ route('books.package_destroy',$item->id) }}"
                                                        method="post">
                                                        @csrf
                                                        @method('delete')
