...
class BookCategoryController extends Controller
{
    ...
    {
        $categories = Category::where('for_book_or_product','1')->orderBy('order_by')->get();
        return view("admin.pages.book.category", compact('categories'));
    }
    ...
    public function showOnNav(Request $request){
        $id = $request->id;
        $category = Category::where('id', $id)->first();
        if(!$category){
            return response()->json([
        ...
        $homePage = NavItem::where('for_book_or_product', '1')
        ->where('item_type','1')->where('item_id',$category->id)->first();
        if(!$homePage){
